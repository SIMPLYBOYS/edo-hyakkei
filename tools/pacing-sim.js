...
function simPub(D) {
  ...
  while (left && day < 365 * 12) {
    // 季節一律問遊戲的曆法，不要在這裡自己算一套。2026/08/08 踩過：
    // 遊戲改成用真實月份分季之後，這裡還在用 365/4 等分，模擬的是另一個遊戲。
    const name = clockFrom(day).season;
...

{
  ...
  const cur = simPub(8);
  console.log(`\n→ 現行 D=8：${cur.years.toFixed(2)} 年，空等 ${cur.idle + cur.waitPub} 日。`);
  console.log('  8 不是調出來的，是量的：廣重 118 枚花 32 個月 ≈ 8.3 日一枚。');
  console.log('  卡住的是春——42 景 × D 日要塞進每年約 89 天的春天，D 一大就多吃一個春。');
  console.log('  這也是為什麼起點必須落在春天第一天（見 src/calendar.js 的 START）。');
  assert.equal(simPub(8).left, 0, '模型C：D=8 收不完');
  assert.ok(simPub(9).years > simPub(8).years, '模型C：D 越大該花越久');
}
